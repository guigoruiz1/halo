...
class TestHalo(unittest.TestCase):
    ...
    def test_text_ellipsing(self):
        ...
        self.assertEqual(
            output[0], "{} {} (...)".format(frames[0], text[: terminal_width - 6 - 2])
        )
        self.assertEqual(
            output[1], "{} {} (...)".format(frames[1], text[: terminal_width - 6 - 2])
        )
        self.assertEqual(
            output[2], "{} {} (...)".format(frames[2], text[: terminal_width - 6 - 2])
        )
    ...
    def test_text_animation(self):
        ...
        spinner = Halo(
            text=text, spinner="dots", stream=self._stream, animation="marquee"
        )
        ...
        self.assertEqual(
            output[0], "{} {}".format(frames[0], text[: terminal_width - 2])
        )
        self.assertEqual(
            output[1], "{} {}".format(frames[1], text[1 : terminal_width - 1])
        )
    ...
    def test_spinner_color(self):
        ...
        for color, color_int in COLORS.items():
            ...
            spinner = Halo(color=color, force_color=True, stream=self._stream)
            spinner.start()
            spinner.stop()
